...
function crearTabla() {
    if (objDatos.filter == "") {
        ...
        objDatos.recordsFilter = objDatos.records.filter(
            item => {
                const { id_producto,numero_de_pedido,nombre_cliente,producto,precio,cantidad,fecha_venta} = item;
                if (id_producto.toUpperCase().search(objDatos.filter.toLocaleUpperCase()) != -1) {
                    return item;
                }
                if (numero_de_pedido.toUpperCase().search(objDatos.filter.toLocaleUpperCase()) != -1) {
                    return item;
                }
                if (nombre_cliente.toUpperCase().search(objDatos.filter.toLocaleUpperCase()) != -1) {
                    return item;
                }
                if (producto.toUpperCase().search(objDatos.filter.toLocaleUpperCase()) != -1) {
                    return item;
                }
    ...
    Swal.fire({
        ...
        resultado=>{
            ...
            if (resultado.isConfirmed) {
                API.get("listadoventasuser/eliminarVenta?id="+id).then(
                    data=>{
                        if (data.success) {
                            cargarDatos();
                        } else {
